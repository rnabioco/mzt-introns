...
# get all introns to examine amount of masking
dup_tx <- read.table(file.path(eisa_dir, "duplicated_seqs.tsv"), header = TRUE)
# gtf_fn <- file.path(eisa_dir, "eisaR.gtf")
# gtf <- import(gtf_fn)
# 
# intron_gtf <- gtf[gtf$type == "exon" & str_detect(gtf$ID, "-I[0-9]*$")]
# txdb <-  makeTxDbFromGRanges(gtf)
# tx <- transcripts(txdb)
# intron_tx <- tx[str_detect(tx$tx_name, "-I[0-9]*$")]
# names(intron_tx) <- intron_tx$tx_name

intron_tx_coord <- GRanges(seqnames = names(eisa_seqs), 
                           ranges = IRanges(start = 1, 
                                            end = seqlengths(eisa_seqs)),
                           strand = "+")
seqlevels(intron_tx_coord) <- seqlevels(eisa_seqs)
seqinfo(intron_tx_coord) <- seqinfo(eisa_seqs) 
...
# set some cutoffs for introns to include for quant
...
irl_to_mask <- irl_to_mask[elementNROWS(irl_to_mask) > 1]
