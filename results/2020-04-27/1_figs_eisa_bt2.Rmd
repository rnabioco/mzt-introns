...
dir.create("processed_files_eisa_bt2")
write_tsv(mdata, "processed_files_eisa_bt2/metadata.tsv")
```
...
gene2intron <- read_tsv("annotation_files/eisa/tx2gene.tsv")
```
...
outdir <- "processed_files_eisa_bt2"
dir.create(outdir)
...
save_plot(file.path(fig_dir, 
                    "ma_eisa_bt2_plot_salmon.pdf"), plt, nrow = 2, ncol = 2)
...
mean_tpms <- read_tsv("processed_files_eisa_bt2/rissland_eisa_bt2_tpm_means.tsv.gz")
...
```{r}
write_tsv(gene_class_df, "processed_files_eisa_bt2/eisa_bt2_gene_classes.tsv")
```
