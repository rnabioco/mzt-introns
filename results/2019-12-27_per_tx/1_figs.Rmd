...
fa_types <- c(
  "primary_transcripts_per_tx_masked"
)
...
```{r}
ris_res <- filter(ris_res, fasta_type == "primary_transcripts_per_tx_masked")
...
# b_genes <- b_dat %>% 
#   dplyr::select(GENES:CELLULARIZED_BLASTODERM) %>% 
#   pivot_longer(cols = -GENES, names_to = "stage") %>% 
#   filter(value) %>% 
...
# exon_zyg <- tibble(gene = zygotic_genes_exon, 
#        type = "mRNA") 
# 
# intron_zyg <- tibble(gene = no_pre(zygotic_genes_intron), 
#        type = "pre-mRNA")
# 
# gene_lists <- list(
# both = intersect(zygotic_genes_exon, no_pre(zygotic_genes_intron)),
# mRNA = setdiff(zygotic_genes_exon, no_pre(zygotic_genes_intron)),
# premRNA = setdiff(no_pre(zygotic_genes_intron), zygotic_genes_exon)
# )
# gene_df <- imap_dfr(gene_lists,
#                ~tibble(gene_id = .x,
#                        type = .y))
# 
# n_genes_per_cat <- group_by(b_genes, stage) %>% 
#   summarize(n_genes = n())
# 
# left_join(gene_df, b_genes, by = "gene_id") %>% 
#   left_join(n_genes_per_cat) %>% 
#   filter(!is.na(stage)) %>% 
#   group_by(stage, type) %>% 
#   summarize(n = n(), 
#             prop = n / unique(n_genes)) %>% 
