...
gtf_dat <- tidy_gtf(gtf_file)

expr_genes <- read_excel("../../docs/bartel_2019_s1.xlsx", skip = 2) %>% 
  left_join(gtf_dat, by = c("GENES" = "gene_name")) %>% 
  pull(gene_id) %>% 
  unique()

splice_windows <- map(splice_regions, 
                      ~filter(.x, gene_id %in% expr_genes))
```
...
imap(plts, ~save_plot(file.path(fig_dir, 
                                paste0("intron_metagene_", .y, ".pdf")), 
                      .x, base_width = 6.5))
