...
gene_class_df <- read_tsv(file.path(results_dir, 
                   "2019-12-27_per_tx", 
                   "processed_files", "gene_classes.tsv"))
...
group_by(gene_classes, mzt_class) %>% 
  mutate(n_genes = n()) %>% 
  ungroup() %>% 
  filter(qval < 0.05) %>% 
  group_by(mzt_class) %>% 
...
res <- mutate(res, 
              count_type = ifelse(str_detect(row, "^pre_"),
                                  "intron",
                                  "exon"),
...
group_by(switching_isos, tx_class, sig, count_type) %>% 
  summarize(n = n())

filter(switching_isos, tx_class == "zygotic", sig == "up", count_type == "exon") %>% 
  View()
```
