...
write_tsv(annots, file.path("output_files",
...
```{r}
sup_url <- "https://www.cell.com/cms/10.1016/j.celrep.2018.07.039/attachment/d3cd58a0-6a77-48f9-bca8-9f31bf42295a/mmc3.xlsx"

download.file(sup_url, "Burow_et_al_table_s2.xlsx")

table_s2 <- readxl::read_excel("Burow_et_al_table_s2.xlsx")

# copied from figure 1c 
optimal_codons <- c(
  "AAG",
  "CGT",
  "GCT",
...
changed_cds <- filter(annots, !same_CDS)

shared_annots <- inner_join(changed_cds,table_s2, by = c("featureID" = "FBtr")) 

calc_percent_optimal <- function(cds_seq, 
                                 optimal_codon_seqs) {
   stringr::str_match_all(cds_seq, ".{3}") %>%
    purrr::map_dfr(~tibble(n_optimal = sum(.x %in% optimal_codon_seqs),
                           n_codons = length(.x),
                           proportion_optimal = n_optimal / n_codons))
}

shared_annots <- mutate(shared_annots, 
                        optimality = calc_percent_optimal(seq,
                                                     optimal_codons)$proportion_optimal)

changed_cds <- mutate(changed_cds, optimality = calc_percent_optimal(seq,
                                                 optimal_codons)$proportion_optimal)

changed_cds <- group_by(changed_cds, gene_id) %>% 
  mutate(delta = log2(nth(optimality, 1) / nth(optimality, 2)))

library(ggbeeswarm)

p <- ggplot(changed_cds, aes(tx_class, optimality)) + 
  geom_quasirandom() +
  geom_boxplot(alpha = 0.5, coef = 100) +
  labs(x = "",
       y = "Percent Optimality")

p_delta <- ggplot(changed_cds, aes("Zygotic/Maternal", delta)) + 
  geom_quasirandom() +
  geom_boxplot(alpha = 0.5, coef = 100) +
  labs(x = "",
       y = "Log2 ratio Percent Optimality")

p <- plot_grid(p, p_delta, nrow = 1, ncol = 2)
p
save_plot("output_files/codon_optimality.pdf", p, nrow = 1, ncol = 2)

ggplot(shared_annots, aes(tx_class, `Embryo half-life`)) +
  geom_quasirandom() +
  scale_y_log10()
```


Find codon changes between sequences not found in maternal

```{r, eval = FALSE}
changed_cds <- filter(annots,!same_CDS)
z_txs <- filter(changed_cds, tx_class == "zygotic")
z_txs <- filter(gtf, type == "CDS",
                transcript_id %in% z_txs$featureID) %>%
  group_by(strand, gene_id, phase) 

m_txs <- filter(changed_cds, tx_class == "maternal")
m_txs <-filter(gtf, type == "CDS", 
         transcript_id %in% m_txs$featureID) %>%
  group_by(strand, gene_id, phase)

# get unique CDS sequence
new_seqs <- bed_subtract(z_txs, m_txs) %>%
  get_sequences(fasta_path = fa_fn)

seq_overlap <- bed_intersect(z_txs, m_txs, 
                             suffix = c("_z", "_m")) %>% 
  mutate(start_m = pmax(start_z, start_m),
         end_m = pmin(end_z, end_m)) %>% 
  rowwise() %>% 
  mutate(in_frame = start_m %in% seq(start_z, 
                                     end_z, by = 3)) %>% 
  ungroup()

seq_overlap <- split(seq_overlap, 1:nrow(seq_overlap))
```
...
optimal_zygo <- calc_percent_optimal(map_chr(seq_diffs_zygotic$new_codons,
                             paste0, collapse = ""), 
                     optimal_codon_seqs = optimal_codons)

optimal_mat <- calc_percent_optimal(map_chr(seq_diffs_maternal$new_codons,
                             paste0, collapse = ""), 
                     optimal_codon_seqs = optimal_codons)

optimal_seq_calcs <- bind_rows(list(`Zygotic isoforms` = optimal_zygo,
                                     `Maternal isoforms` = optimal_mat),
                               .id = "class") %>% 
  na.omit()
...
p <- ggplot(optimal_seq_calcs, aes(class, proportion_optimal)) + 
  geom_quasirandom() +
  geom_boxplot(alpha = 0.5, coef = 100) +
  labs(x = "",
       y = "Percent Optimality")
...
save_plot("output_files/codon_optimality_new_sequence.pdf", p, base_asp = 1.0)
```

```{r}
seq_diffs <- bind_rows(list(`Zygotic isoforms` = seq_diffs_zygotic ,
                                     `Maternal isoforms` = seq_diffs_maternal),
                               .id = "tx_class") 

aa_stats <- split(seq_diffs, seq_diffs$tx_class) %>% 
  map_dfr(~str_split(.x$new_aa, "") %>%
            unlist() %>% tibble(aa = .) %>% 
            filter(aa != "*") %>% 
            group_by(aa) %>% 
            summarize(n_aa = n()) %>% 
            ungroup() %>%
            mutate(proportion = n_aa / sum(n_aa)), 
          .id = "tx_class")

p <- ggplot(aa_stats, 
       aes(reorder(aa, -proportion), proportion)) +
  geom_col(aes(fill =tx_class), 
           position = "dodge") +
  scale_fill_brewer(name = "",
                    palette = "Set1") +
  labs(x = "") +
...
p
save_plot("output_files/aa_usage_new_sequence.pdf", p, base_asp = 1.5)
...
codon_stats <- split(seq_diffs, seq_diffs$tx_class) %>% 
  map_dfr(~unlist(.x$new_codons) %>%
            tibble(codons = .) %>% 
            group_by(codons) %>% 
            summarize(n_codons = n()) %>% 
            ungroup() %>%
            mutate(proportion = n_codons / sum(n_codons)), 
          .id = "tx_class")

codon_stats <- mutate(codon_stats,
                      aa = GENETIC_CODE[codons],
                      codon_str = paste0("(", aa, ") ", codons ),
                      optimal = ifelse(codons %in% optimal_codons,
                                       "Optimal",
                                       "Not Optimal"),
                      optimal = factor(optimal, levels = c("Optimal",
                                                           "Not Optimal")))

p <- ggplot(codon_stats, 
       aes(reorder(codon_str, -proportion), proportion)) +
  geom_col(aes(fill =tx_class), 
           position = "dodge") +
  facet_grid(~optimal, drop = TRUE, scales = "free_x", space = "free_x") + 
  scale_fill_brewer(name = "",
                    palette = "Set1") +
  labs(x = "") +
  theme(
...
p
save_plot("output_files/codon_usage_new_sequence.pdf", p, base_asp = 2.5)
...
```{r mass_spec}
peps <- read.table("~/Downloads/DeJung.2018.2.9.peptides_timecourse.txt.gz", 
                   sep = " ",
                   stringsAsFactors = FALSE) 

peps <- select(peps, Sequence, 
...
# matched_peps <- map(split(seq_diffs, 1:nrow(seq_diffs))[1], 
#     ~peps[str_which(.x$new_aa, peps$Sequence), 
#           ])
...
additional_annots <- left_join(annots,
                               summary_matched_peps, by = c("gene_id", "gene_name"))

write_tsv(additional_annots, "isoform_summaries.tsv.gz")
```
...
tissue_expr <- read_tsv("gene_rpkm_report_fb_2017_05.tsv.gz", skip = 5)

tissue_expr <- filter(tissue_expr, Parent_library_name == "modENCODE_mRNA-Seq_tissues")

tissue_expr$RNASource_name <- str_remove(tissue_expr$RNASource_name, "mE_mRNA_")
```
## Motifs
...
```{r}
delta_cds <- filter(annots, !same_CDS)
...
filter(delta_cds, 
       tx_class == "zygotic") %>% 
  write_fasta(., 
              file.path(fa_dir, "zygotic_cds_nt.fasta"), 
              header = "featureID", seq_col = "seq")

filter(delta_cds, 
       tx_class == "maternal") %>% 
  write_fasta(., 
              file.path(fa_dir, "maternal_cds_nt.fasta"), 
              header = "featureID", seq_col = "seq")

```

### Transcript level differences (kmer)

```{r}
kmer_enrichment <- function(gtf,
                            feature = "exon",
                            type = "transcript_id",
                            group_1,
                            group_2,
  ...
  filtered_gtf <- filter(gtf, !!sym(type) %in% union(group_1, group_2))
         
  tx_seqs <- txcompare::gtf_to_seq(filtered_gtf, 
                      fasta_path = fasta_fn, 
                      type_field = feature)
  
  kmers_all <- mutate(tx_seqs, 
                      kmers = get_kmers(seq, 
                                        n = k, 
                                        both_strands = FALSE)) %>% 
                select(!!sym(type), kmers) %>% 
  unnest() %>% 
  ungroup()

  bg_count <- kmers_all %>% 
      filter(!!sym(type) %in% group_2) %>% 
      mutate(success = ifelse(counts >= 1, 1L, 0L)) %>% 
      group_by(kmer) %>% 
      summarize(counts = sum(success))
  
  test_count <- kmers_all %>% 
      filter(!!sym(type) %in% group_1) %>% 
      mutate(success = ifelse(counts >= 1, 1L, 0L)) %>% 
      group_by(kmer) %>% 
      summarize(counts = sum(success))
  
  kmers_total <- left_join(test_count, 
                           bg_count,
                           by = "kmer",
                           suffix = c("_test", "_bg"))
  
  kmers_total <- mutate(kmers_total, 
                        n_seq_test = length(unique(group_1)),
                        n_seq_bg = length(unique(group_2))) %>% 
    na.omit()
...
```{r}
delta_any <- filter(annots, !same_CDS | !same_three_prime_utr | !same_five_prime_utr)

z_txs <- filter(delta_any, tx_class == "zygotic") %>% pull(featureID)
m_txs <- filter(delta_any, tx_class == "maternal") %>% pull(featureID)

a <- kmer_enrichment(gtf = gtf, 
                     feature = "exon",
                     type = "transcript_id",
                     group_1 = z_txs, 
                     group_2 = m_txs, 
...
delta_any <- filter(annots, !same_three_prime_utr)
...
filtered_gtf <- filter(gtf, transcript_id %in% z_txs)
         
z_tx_seqs <- txcompare::gtf_to_seq(filtered_gtf, 
                      fasta_path = fa_fn, 
                      type_field = "CDS")

filtered_gtf <- filter(gtf, transcript_id %in% m_txs)
         
m_tx_seqs <- txcompare::gtf_to_seq(filtered_gtf, 
                      fasta_path = fa_fn, 
                      type_field = "CDS")

library(transite)
foreground.kmers <- generateKmers(z_tx_seqs$seq, 6)
background.kmers <- generateKmers(m_tx_seqs$seq, 6)
