...
walk(c("tx-ribo-coverage-plots", "tx-nano-plots", "nano-read-align", "tx-coverage-plots"),
...
ont_align_summary <- read_tsv(file.path("output_files",
...
walk(to_plot[10], 
    ~{
      ...
      pdf(file.path("../figures/pdfs/tx-nano-plots/", str_c(.x, "_coverage.pdf")),
          width = 12,
          height = 4)
      plot_ont_tx(ont_align, features, gtf_obj, cols_to_use = scbp::palette_OkabeIto,
                 title.width = 1.5,
...
ont_align_summary <- read_tsv("output_files/all_merged_read_alignments.tsv")
```
...
nano_tx_outdir <- "../figures/pdfs/nano-read-align/"
dir.create(nano_tx_outdir, showWarnings = FALSE)
walk(to_plot,
    ~{
      features <- switch_isos %>%
        ...
        pull(transcript_id) 
      pdf(file.path(nano_tx_outdir, str_c(.x, "_coverage.pdf")), height = 5, width = 12)
  ...
  p <- ont_align_summary %>% 
    ...
    ggplot(aes(sample)) + 
      ...
      labs(x = "",
           y = "# of reads",
           fill = "")
  save_plot(file.path(nano_tx_outdir, str_c(.x, "_read_counts.pdf")),
            p,
            base_asp = 1)
})
...
base_dir <- here(data_dir, "bigwigs/drosophila/star/RISSLAND")
...
Include ribosome profiling and nanopore
...
```{r}
bw_dir <- here("data/bigwigs/drosophila/bt2/rpf/EICHHORN/")
ribo_bw_fns <- dir(file.path(bw_dir), 
              pattern = "fwd.bw", 
              full.names = TRUE)
rpf_mdata <- read_tsv("~/Projects/mzt-introns/data/raw_data/drosophila/EICHHORN/PRJNA326506.txt")
...
tx_figs <- file.path(fig_dir, "tx_ribo_figs")
dir.create(tx_figs, showWarnings = FALSE)
...
read_bb <- function(bb_fn, 
                    region = NULL,
                    strand_to_keep = NULL,
                    bin = "/Users/kriemo/bin/kent/bigBedToBed"){
