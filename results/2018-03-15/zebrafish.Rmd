...
res_mature_lrt_tidy <- results(dds_mature, tidy = TRUE, contrast = c("stageName", "Shield", "1_cell"))

dds <- DESeqDataSetFromTximport(pres, pdata, ~stageName)
dds <- dds[ rowSums(counts(dds)) > 100, ] 
dds_pre <- DESeq(dds, test = "LRT", reduced = ~ 1 )
res_pre <- results(dds_pre)
res_pre_lrt_tidy <- results(dds_pre, tidy = TRUE, contrast = c("stageName", "Shield", "1_cell"))
```
...
pas_clusters <- inner_join(clusters, pas, by = c("gene_name" = "Gene name"))
