...
gene2intron <- read_tsv(file.path(project_dir, "dbases-rerun", "drosophila/eisa/tx2gene.tsv")) 
```
...
```{r}
count_data_dir <- here("data-rerun", 
                            "featurecounts",
                            "drosophila",
                            "RISSLAND")
...
mean_exonic_tpms <- mean_exonic_tpms %>% 
  ...
  summarize(expr_tpt = stage[expressed][1], # get first tpt above tpm threshold
            max_tpm  = max(tpm, na.rm = TRUE),
            mean_tpm = mean(tpm, na.rm = TRUE)) %>% 
  left_join(mean_exonic_tpms, ., by = "gene_name")  %>% 
    ungroup() %>% 
  select(-gene_name)
...
mean_intronic_tpms <- mean_intronic_tpms %>% 
  ...
  summarize(expr_tpt = stage[expressed][1], # get first tpt above tpm threshold
            max_tpm  = max(tpm, na.rm = TRUE),
            mean_tpm = mean(tpm, na.rm = TRUE)) %>% 
  left_join(mean_intronic_tpms, ., by = "gene_name") %>% 
...
```
Note that the #s of genes shown here are fewer than the # of genes that we can classify. For the `classification` panel, the # is less because there are genes that do not have zelda binding information in the Eisen data, due to gene name differences or other reasons. The `Zygotic genes` panel has fewer genes because we classify time to expression based solely on intronic read counts, which are not present for every zygotic gene (due to monoexonic genes or no intronic signal for other reasons), in addition to the issue previously mentioned.


## Compare intron supplemented salmon counts to original spliced only reference

```{r, fig.height=7, fig.width=3.5}
masked_files <- here("data-rerun",
                   "salmon_bt2_masked", 
                   "drosophila",
                   pdata_split$RISSLAND$study,
                   fa_type,
...
uniq_kmers <- read_tsv(here('dbases-rerun', 'drosophila', 'eisa_uniqueness.tsv')) %>%
