...
library(txcompare)
library(valr)
gtf <- txcompare::tidy_gtf(annots$drosophila$gtf)
...
```{r}
bw_dir <- "~/Projects/mzt-introns/data/bigwigs"
ribo_bw_fns <- dir(file.path(bw_dir, "drosophila", "EICHHORN"), 
              pattern = "fwd.bw", 
              full.names = TRUE)
...
ribo_bw_fns_fwd <- file.path(bw_dir,  "drosophila", "EICHHORN",
                 paste0(rpf_mdata$run_accession, "_fwd.bw"))
names(ribo_bw_fns_fwd) <- rpf_mdata$sample_title
ribo_bw_fns_rev <- file.path(bw_dir, "drosophila", "EICHHORN",
