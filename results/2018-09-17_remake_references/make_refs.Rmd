...
fasta_fn <- '~/Projects/shared_dbases/genomes/drosophila/Drosophila_melanogaster.BDGP6.dna.toplevel.fa'
...
bws <- dir(
  file.path(data_dir, "bigwigs", "drosophila", "RISSLAND"),
  full.names = TRUE,
  pattern = ".bw"
)
...
gtf_fn <- file.path(project_dir, "dbases", "drosophila", "primary_transcripts.gtf")
gtf <- import(gtf_fn)
...
bed_df <- read_bed("fly_intron_mask.bed", n = 6)
...
bed_df <- read_bed("fly_exon_intron_plus_exons_juncts_mask.bed", n = 6)
bed_df <- mutate(bed_df, start = start + 1)
...
covered_intron_mask_bed <- "fly_intron_mask_txcoords.bed"
covered_intron_exon_mask_bed <- "fly_intron_mask_plus_exon_txcoords.bed"
...
Sys.setenv(INTRON_FA = file.path(project_dir, "dbases", "drosophila", "primary_transcripts_masked.fa"))
Sys.setenv(INTRON_EXON_FA = file.path(project_dir, "dbases", "drosophila", "primary_transcripts_masked_exons.fa"))

```

```{bash primary_transcript_fa}
echo $FA
head $INTRON_BED
